...
\ and the deviation by temp. and VCC to get the approximate freq. of the internal
\ oscillator. needs ADC driver and compensation algorithms.
	NominalHSIFreq
;
...
: SystemCoreClockUpdate ( -- )
	GetSysClock SystemCoreClock !
;

: rescaleToClock ( n -- n )
	GetSysClock SystemCoreClockOld */
;
...
: SetSysClockToPll
	...
	until 
	0= if ." PLL is not sysclock" cr 0 exit then
	-1
;
...
: SetSysClock 
	...
	TurnPllClockOn not if
		...
		." PLL frequency: " GetPllSrcFreq pll_m @ * . cr
	then
