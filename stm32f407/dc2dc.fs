...
: tim3-init ( -- )
	...
	$1060 TIM3_BASE TIM1_CCMR1 + h! \ CH1 PWM output, CH2 High on Match
	$0001 TIM3_BASE TIM1_CCER + h!	\ enable CH1 output
	0 TIM3_CCR1 + !
	19 TIM3_BASE TIM1_CCR2 + h!
	4 TIM3_DIER h!	\ enable CCR2 interrupt
...
: tim3-isr
	1 Tim3-count +!
	4 TIM3_SR bic! 			\ clear interrupt source
	analog
	Vout-samp
	...
	begin
		Tim3-count @ 10 >= if
			...
			Vout-samp-arr-idx @ 0 > if
				...
				\ store tha calculated value to TIM3_CCR1
				TIM3_CCR1 h!
			then
		then
	again
